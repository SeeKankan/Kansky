...
import io.seekankan.github.kansky.util.ItemNBTProxy;
...
//            }
//        });
        NBT.modify(event.getItemDrop(), ItemNBTProxy.class, proxy -> {
            if(!proxy.hasOwner()) {
                proxy.setOwner(proxy.hasThrower() ? proxy.getThrower() : event.getPlayer().getName());
            }
        });
    }
}
